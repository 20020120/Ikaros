...
PsOutPut main(PsInput psIn)
{
 ...
 //       // 最終的な拡散反射光を計算する
	//	const float3 diffuse = albedoColor.xyz * p;
 ...
 //       // 滑らかさが高ければ、拡散反射は弱くなる
	...
	{
		// ライトの向きを計算
		float4 Direction = psIn.WorldPosition - PLightPosition[i];
		...
		const float3 lambertDiffuse = PLightColor[i].xyz * NdotL / PI;
		...
		// ライトの距離に夜よる減衰を計算する
        const float3 Dist = length(psIn.WorldPosition.xyz - PLightPosition[i].xyz);
	...
	//--------------------<スポットライトを計算する>--------------------//
	{
		float3 Direction = psIn.WorldPosition.xyz - SLightPosition.xyz;
        ...
        // 正規化Lambert拡散反射を求める
		const float NdotL = saturate(dot(normal, -Direction.xyz));
		const float3 lambertDiffuse = SLightColor.xyz * NdotL / PI;
		...
		// ライトの距離に夜よる減衰を計算する
		const float3 Dist = length(psIn.WorldPosition.xyz - SLightPosition.xyz);
	...
	PsOut.Luminaunce_RenderTarget.rgb = EmissiveColor.rgb * 5.0f;
	PsOut.Luminaunce_RenderTarget.a = 1.0f;
