...
PsOutPut main(PsInput psIn)
{
	...
	albedoColor.x = lerp(albedoColor.x, PLightColor2.x, step(2.9f, WhiteChecker));
	albedoColor.y = lerp(albedoColor.y, PLightColor2.y, step(2.9f, WhiteChecker));
	albedoColor.z = lerp(albedoColor.z, PLightColor2.z, step(2.9f, WhiteChecker));
...

	{
        ...
        // 最終的な拡散反射光を計算する
		const float3 diffuse = albedoColor.xyz * lambertDiffuse;
