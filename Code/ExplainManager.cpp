 ...
 void ExplainManager::RenderAttackExp(ID3D11DeviceContext* dc)
{
     ...
     case Controller:
         ...
         font->Begin(dc);
         // 射撃で敵を倒す
         font->Draw({ 890.0f,45.0f }, { 0.5f,0.5f }, { 1.0f,1.0f,1.0f,1.0f }, L"射撃で敵を倒す(Bボタン)", wcslen(L"射撃で敵を倒す(Bボタン)"), Font::TEXT_ALIGN::UPPER_LEFT);
 ...
 void ExplainManager::RenderDodgeExp(ID3D11DeviceContext* dc)
{
     ...
     case Controller:
     {
         ...
         font->Begin(dc);
         // 攻撃をギリギリで回避する
         font->Draw({ 890.0f,45.0f }, { 0.5f,0.5f }, { 1.0f,1.0f,1.0f,1.0f }, L"弾をギリギリで回避する(Yボタン)", wcslen(L"弾をギリギリで回避する(Yボタン)"), Font::TEXT_ALIGN::UPPER_LEFT);
 ...
 void ExplainManager::RenderElement2Exp(ID3D11DeviceContext* dc)
{
     ...
     case Controller:
         ...
         font->Begin(dc);
         font->Draw({ 890.0f,45.0f }, { 0.5f,0.5f }, { 1.0f,1.0f,1.0f,1.0f }, L"ゲージを溜めて", wcslen(L"ゲージを溜めて"), Font::TEXT_ALIGN::UPPER_LEFT);
         font->Draw({ 890.0f,85.0f }, { 0.5f,0.5f }, { 1.0f,1.0f,1.0f,1.0f }, L"Rボタン/トリガーで属性を変える", wcslen(L"Rボタン/トリガーで属性を変える"), Font::TEXT_ALIGN::UPPER_LEFT);
         font->End(dc);
