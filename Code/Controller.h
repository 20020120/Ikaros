...
public:
	...
	// 更新
	void Update(float elapsedTime_ =0.0f);
	...
	// コントローラーに接続中かの取得
	bool IsPadConnecting() const { return is_pad_connecting; }

    // バイブレーション
	bool SetVibration(float R, float L, float StopTime_);
void SetFirstUpdate()
{
	FirstUpdate = false;
}
private:
	...
	float				triggerR = 0.0f;
	int					slot = 0;
	bool				is_pad_connecting = false;

	float Stack_StopVibSec;
	float VibStackSec;
