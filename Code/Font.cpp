...
Font::Font(ID3D11Device* device, const char* filename, int maxSpriteCount)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "resources\\Shaders\\Font_VS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "resources\\Shaders\\Font_PS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
void Font::Draw(float x, float y, const wchar_t* string, size_t length, TEXT_ALIGN align)
{
	size_t DrawString = 0;
	...
	for (size_t i = 0; i < length; ++i)
	{
		...
		else if (code == CharacterInfo::ReturnCode)//改行
		{
		...
		//行の長さを取得
		size_t l = DrawString + lineLength + 1;
		size_t d = DrawString;
		float slength = 0.0f;
		for (d; d < l; d++)
		{
			WORD sword = static_cast<WORD>(string[d]);
			WORD scode = characterIndices.at(sword);
			if (scode != CharacterInfo::ReturnCode && scode != CharacterInfo::EndCode && scode != CharacterInfo::TabCode && scode != CharacterInfo::SpaceCode)
			{
				const CharacterInfo& info = characterInfos.at(scode);
				slength += info.xadvance;
			}
		}

		//描画開始位置取得
		DirectX::XMFLOAT2 drawPos = AdjustTextOrigin(align, { start_x,y }, slength, fontHeight);

		//描画
		for (DrawString; DrawString < l; ++DrawString)
		{
			// 文字値から文字情報が格納されているコードを取得
			WORD drawWord = static_cast<WORD>(string[DrawString]);
			WORD drawCode = characterIndices.at(drawWord);

			// 特殊制御用コードの処理
			if (drawCode == CharacterInfo::EndCode)
			{
				break;
			}
			else if (drawCode == CharacterInfo::ReturnCode)
			{
				continue;
			}
			else if (drawCode == CharacterInfo::TabCode)
			{
				drawPos.x += space * 4;
				continue;
			}
			else if (drawCode == CharacterInfo::SpaceCode)
			{
				drawPos.x += space;
				continue;
			}
			...
			// 文字情報を取得し、頂点データを編集
			const CharacterInfo& info = characterInfos.at(drawCode);

			float positionX = drawPos.x + info.xoffset;// + 0.5f;
			float positionY = drawPos.y + info.yoffset;// + 0.5f;
...
void Font::Draw(DirectX::XMFLOAT2 pos, const DirectX::XMFLOAT2 scale, const DirectX::XMFLOAT4 color, const wchar_t* string, size_t length, TEXT_ALIGN align)
{
#if 1
	float start_x = pos.x;
	float start_y = pos.y;
	...
	{
		// 文字値から文字情報が格納されているコードを取得
		WORD drawWord = static_cast<WORD>(string[i]);
		WORD drawCode = characterIndices.at(drawWord);
		const CharacterInfo& info = characterInfos.at(drawCode);
		...
		{
			lineLength += static_cast<size_t>(4 * space);
			continue;
		}
		else if (drawCode == CharacterInfo::SpaceCode)
		{
			lineLength += static_cast<size_t>(space);
			continue;
		}
		else
		{
			lineLength += info.width;
		}
	}

	//描画開始位置取得
	DirectX::XMFLOAT2 drawPos = AdjustTextOrigin(align, { start_x,pos.y }, lineLength * scale.x, fontHeight * scale.y);

	//描画
	for (size_t DrawString = 0; DrawString < length; ++DrawString)
	{
		// 文字値から文字情報が格納されているコードを取得
		WORD drawWord = static_cast<WORD>(string[DrawString]);
		WORD drawCode = characterIndices.at(drawWord);
		...
		drawPos.x += info.xadvance * scale.x;
	...
	for (size_t i = 0; i < length; ++i)
	{
		...
		for (d; d < l; d++)
		{
			...
			{
				const CharacterInfo& info = characterInfos.at(scode);
				slength += info.xadvance * scale.x;
			}
		}

		//描画開始位置取得
		DirectX::XMFLOAT2 drawPos = AdjustTextOrigin(align, { start_x,pos.y }, slength, fontHeight * scale.y);
...
void Font::End(ID3D11DeviceContext* context)
{
	...
	if (size <= 0)return;//描画する文字量が0以下なら描画しない
...
DirectX::XMFLOAT2 Font::AdjustTextOrigin(TEXT_ALIGN align, DirectX::XMFLOAT2 pos, float width, float height)
{
	switch (align)
	{
	default:
