...
// 更新
void GamePad::Update(float elapsedTime_)
{
	if(FirstUpdate)
	{
		int a = 0;
	...
	Stack_StopVibSec += elapsedTime_;
	if (VibStackSec <= Stack_StopVibSec)
	{
	...
	DWORD temp = XInputGetState(slot, &xinputState);
	if (temp == ERROR_SUCCESS)
	{
		...
		is_pad_connecting = true;
	}

	else if(temp == ERROR_DEVICE_NOT_CONNECTED)
	{
		is_pad_connecting = false;
	}
	else
	{
#if 0
	...
	// キーボードでエミュレーション
	{
		...
		if (GetAsyncKeyState('K') & 0x8000) newButtonState |= BTN_A;
		if (GetAsyncKeyState('L') & 0x8000) newButtonState |= BTN_B;
		if (GetAsyncKeyState('V') & 0x8000) newButtonState |= BTN_X;
		if (GetAsyncKeyState('J') & 0x8000) newButtonState |= BTN_Y;
		...
		if (GetAsyncKeyState(VK_SPACE) & 0x8000)	newButtonState |= (BTN_RIGHT_TRIGGER | BTN_X);
		if (GetAsyncKeyState(VK_RETURN) & 0x8000)	newButtonState |= BTN_X;
...
bool GamePad::SetVibration(float R, float L, float StopTime_)
{
	Stack_StopVibSec = 0.0f;
	VibStackSec = StopTime_;
